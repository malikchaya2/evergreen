...
import (
	...
	"github.com/evergreen-ci/evergreen/graphql/models"
...
func (r *patchResolver) VariantTasks(ctx context.Context, obj *patch.Patch) ([]*models.VariantTask, error) {
	variantTasks := []*models.VariantTask{}
	for _, vt := range obj.VariantsTasks {
		vtasks := make([]*string, 0)
		for _, task := range vt.Tasks {
			vtasks = append(vtasks, &task)
		}
		variantTasks = append(variantTasks, &models.VariantTask{
