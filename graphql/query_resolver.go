...
func (r *queryResolver) TaskQueueDistros(ctx context.Context) ([]*TaskQueueDistro, error) {
	...
	for _, distro := range queues {
		// pass in a new context here
		taskQueueDistrosCtx, cancel := context.WithTimeout(context.Background(), timeout)
		defer cancel()
		numHosts, err := host.CountRunningHosts(taskQueueDistrosCtx, distro.Distro) // here
		if err != nil {
