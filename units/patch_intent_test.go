...
// 			s.Equal(currentPatchDoc.BuildVariants, []string{"bv1", "bv_only_dt", "bv_different_dt"})
...
// 	tasks := []task.Task{
// 		{
// 			Id:           "t1", // should only include this one
...
// 	s.Equal([]string{"t1"}, currentPatchDoc.Tasks)
// }
...
// 	s.NoError(err)
// 	sort.Strings(currentPatchDoc.Tasks)
// 	s.Equal(previousPatchDoc.Tasks, currentPatchDoc.Tasks)
...
func (s *PatchIntentUnitsSuite) TestBuildTasksAndVariantsWithReusePatchId() {
	...
	tasks := []task.Task{
		...
		{
			Id:           "test-agent-command",
			Activated:    true,
			BuildVariant: "ubuntu2204-2",
			BuildId:      "b0",
			Version:      earlierPatchId,
			Status:       evergreen.TaskSucceeded,
			Project:      s.project,
			DisplayName:  "test-agent-command",
		},
		{
			Id:           "test-agent-internal-client",
			Activated:    true,
			BuildVariant: "ubuntu2204",
			BuildId:      "b1",
			Version:      earlierPatchId,
			Status:       evergreen.TaskSucceeded,
			Project:      s.project,
			DisplayName:  "test-agent-internal-client",
		},
	}
	...
	earlierPatchDoc := &patch.Patch{
		...
		Author:        s.user,
		Version:       earlierPatchId,
		Tasks:         []string{"test-cloud", "test-agent-command", "test-agent-internal-client"},
		BuildVariants: []string{"ubuntu2204-2", "ubuntu2204"},
		VariantsTasks: []patch.VariantTasks{
			{
				Variant: "ubuntu2204-2",
				Tasks:   []string{"test-agent-command"},
			},
			{
				Variant:      "ubuntu2204",
				DisplayTasks: []patch.DisplayTask{{Name: "display_task"}},
				Tasks:        []string{"test-agent-internal-client", "test-cloud"},
			},
		},
	}
