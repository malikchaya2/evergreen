...
// It returns the failed bucket config to set on the task's output info.
func (t *Task) moveObjectKeysToFailedBucket(ctx context.Context, srcCfg evergreen.BucketConfig, creds aws.CredentialsProvider, keys []string) (evergreen.BucketConfig, error) {
	failedCfg := getFailedBucketConfig()

	// Buckets
	srcBucket, err := newBucket(ctx, srcCfg, creds)
	if err != nil {
		return evergreen.BucketConfig{}, errors.Wrap(err, "getting source bucket")
	}
	failedBucket, err := newBucket(ctx, failedCfg, creds)
	if err != nil {
		return evergreen.BucketConfig{}, errors.Wrap(err, "getting failed bucket")
	}

	// Move only if there are keys; still return failed config so caller can update output info.
	if len(keys) > 0 {
		if err := srcBucket.MoveObjects(ctx, failedBucket, keys, keys); err != nil {
			return evergreen.BucketConfig{}, errors.Wrap(err, "moving objects to failed bucket")
		}
	}

	return failedCfg, nil
}

// MoveTestLogsToFailedBucket moves all test logs from the regular bucket to the failed bucket for a failed task.
func (task *Task) MoveTestLogsToFailedBucket(ctx context.Context, output *TaskOutput) error {
	...
	return task.moveLogsToFailedBucketHelper(ctx, output.TestLogs.BucketConfig, output.TestLogs.AWSCredentials, keys, func(failedCfg evergreen.BucketConfig) {
...
func (task *Task) MoveTaskLogsToFailedBucket(ctx context.Context, output *TaskOutput) error {
	var keys []string
	...
	return task.moveLogsToFailedBucketHelper(ctx, output.TaskLogs.BucketConfig, output.TaskLogs.AWSCredentials, keys, func(failedCfg evergreen.BucketConfig) {
		task.TaskOutputInfo.TaskLogs.BucketConfig = failedCfg
	})
}

func (t *Task) MoveLogsToFailedBucket(ctx context.Context) error {
	output, ok := t.GetTaskOutputSafe()
	if !ok {
		return nil
	}
	if err := t.MoveTaskLogsToFailedBucket(ctx, output); err != nil {
		return errors.Wrap(err, "moving task logs to failed bucket")
	}
	if err := t.MoveTestLogsToFailedBucket(ctx, output); err != nil {
		return errors.Wrap(err, "moving test logs to failed bucket")
	}
	return nil
}

// getFailedBucketConfig returns the failed bucket config for a project if needed.
func getFailedBucketConfig() evergreen.BucketConfig {
	env := evergreen.GetEnvironment()
	if env != nil && env.Settings() != nil {
		return env.Settings().Buckets.LogBucketFailedTasks
	}
...
func (task *Task) moveLogsToFailedBucketHelper(ctx context.Context, bucketConfig evergreen.BucketConfig, creds aws.CredentialsProvider, keys []string, updateOutput func(failedCfg evergreen.BucketConfig)) error {
	if !ShouldUseFailedBucket(task) {
		return nil
	}
	if len(keys) == 0 {
	...
	failedCfg, err := task.moveObjectKeysToFailedBucket(ctx, bucketConfig, creds, keys)
	if err != nil {
		return err
	}
	updateOutput(failedCfg)
	return nil
}
