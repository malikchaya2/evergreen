...
type APIRepoEvent struct {
	Timestamp *time.Time      `json:"ts"`
	User      *string         `json:"user"`
	Before    APIRepoSettings `json:"before"`
	After     APIRepoSettings `json:"after"`
}
...
func (e *APIRepoEvent) BuildFromService(h interface{}) error {
	switch v := h.(type) {
	case model.ProjectChangeEventEntry:
		...
		before, err := DbRepoSettingsToRestModel(data.Before)
		if err != nil {
			return errors.Wrap(err, "unable to convert 'before' changes")
		}
		after, err := DbRepoSettingsToRestModel(data.After)
		if err != nil {
...
func (e *APIRepoEvent) ToService() (interface{}, error) {
	return nil, errors.New("ToService not implemented for APIRepoEvent")
}

func DbRepoSettingsToRestModel(settings model.ProjectSettings) (APIRepoSettings, error) {
	apiRepoRef := APIProjectRef{}
	if err := apiRepoRef.BuildFromService(settings.ProjectRef); err != nil {
		return APIRepoSettings{}, err
	}

	apiSubscriptions, err := DbRepoSubscriptionsToRestModel(settings.Subscriptions)
	if err != nil {
		return APIRepoSettings{}, err
	}

	apiRepoVars := APIRepoVars{}
	if err := apiRepoVars.BuildFromService(&settings.Vars); err != nil {
		return APIRepoSettings{}, err
	}
...
func (p *APIRepoVars) ToService() (interface{}, error) {
...
func (p *APIRepoVars) BuildFromService(h interface{}) error {
	switch v := h.(type) {
...
func (a *APIRepoAlias) ToService() (interface{}, error) {
...
func (a *APIRepoAlias) BuildFromService(h interface{}) error {
	switch v := h.(type) {
...
func DbRepoAliasesToRestModel(aliases []model.ProjectAlias) []APIRepoAlias {
	result := []APIRepoAlias{}
	for _, alias := range aliases {
		apiAlias := APIRepoAlias{
...
func DbRepoSubscriptionsToRestModel(subscriptions []event.Subscription) ([]APISubscription, error) {
